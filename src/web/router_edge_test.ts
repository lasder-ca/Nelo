...
Deno.test("router compares specificity by segment before registration order", async () => {
  const app = new Nelo();
  app.get("/:section/details", (context) =>
    context.json({ route: "parameter-first", section: context.params.section })
  );
  app.get("/users/:id", (context) =>
    context.json({ route: "static-first", id: context.params.id })
  );

  const response = await app.fetch(new Request("https://example.test/users/details"));
